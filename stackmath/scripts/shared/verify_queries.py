...
def main():
    ...
    prompt_path = args.prompt or (
        ...
        if args.false
        else "stackmath/prompts/stackexchange/verify.md"
    )
    ...
    print(f"Verified keep: {len(kept_ids)} threads: {', '.join(kept_ids)}")
    print(f"Verified reject: {len(rejected_ids)} threads: {', '.join(rejected_ids)}")
