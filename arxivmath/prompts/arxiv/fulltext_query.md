...
11. **All context provided**
    Ensure the question contains all necessary context from the paper to be answerable. In particular, all variables, notation, assumptions, and quantities used in the question must be explicitly defined within the question itself.
    It is okay if questions are long, as long as they remain clear and unambiguous.

12. **Be careful with bounds**
    Some papers prove bounds or inequalities. These are acceptable only if the bound is stated to be tight or exact in the paper, so that there is a unique correct answer.
    Otherwise, such papers should be rejected.
---
