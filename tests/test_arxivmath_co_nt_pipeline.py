...
def test_create_train_category_script_passes_explicit_category_defaults(tmp_path):
    ...
    assert "--paper-root arxivmath/train_math_ap" in calls[1]
    assert "--limit 200" in calls[1]
    assert "verify_queries.py" in calls[2]
    assert "--paper-root arxivmath/train_math_ap" in calls[2]
    assert "--limit 200" in calls[2]
    assert "fulltext_review.py" in calls[3]
    assert "--paper-root arxivmath/train_math_ap" in calls[3]
...
def test_create_train_category_script_supports_multiple_categories_and_overrides(tmp_path):
    ...
    subprocess.run(["bash", str(script_path), "math.CO", "math.NT"], cwd=matharena_root, env=env, check=True)
